...
const PatientsList = () => {
  ...
  const columns = [
    ...
    { field: 'address', headerName: 'Address', width: 200, editable: true },
    {
      ...
      renderCell: (params) => (
        <>
          <IconButton onClick={() => handleViewPatient(params.id)} color="primary">
            <VisibilityIcon />
          </IconButton>
          <IconButton onClick={() => handleDeletePatient(params.id)} color="error">
            <DeleteIcon />
          </IconButton>
        </>
      )
  ...
  const handleViewPatient = async (patientId) => {
    try {
      const patientDetails = await PatientService.getPatientByPatientId(patientId);
  ...
  const confirmDelete = async () => {
    try {
      ...
      const updatedPatients = patientData.filter((patient) => patient.id !== patientToDelete);
  ...
  const handleProcessRowUpdate = async (newRow) => {
    try {
      ...
      const updatedRows = patientData.map((row) => (row.id === newRow.id ? newRow : row));
