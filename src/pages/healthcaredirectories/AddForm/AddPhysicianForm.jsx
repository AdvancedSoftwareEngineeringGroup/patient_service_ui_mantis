...
const AddPhysicianForm = () => {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      await PatientService.createPhysician(values);
      navigate('/physicians'); // Navigate back to the list after successful creation
    } catch (error) {
      console.error('Failed to create physician', error);
      setSubmitting(false);
    }
  };
  if (location.pathname === '/physicians/add') {
    return (
      <>
        <h1>Add Physician</h1>
